...
def get_hh(split: str = 'train', silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
...
def get_ours(file_path):
    ...
    for item in data_list:
        ...
        n_responses = len(data[prompt]['responses'])
        data[prompt]['responses'].extend([good_response, bad_response])
        data[prompt]['pairs'].append((n_responses, n_responses + 1))
        data[prompt]['sft_target'] = good_response
...
def get_batch_iterator(names: str,
                       tokenizer,
                       split: str = 'train',
                       batch_size: int = 1,
                       shuffle: bool = True,
                       ...
                       file_path: Optional[str] = None) -> Iterator[Dict]:
    """Get an iterator over batches of data from a specified dataset or a file.
    ...
    with TemporarilySeededRandom(seed):
        ...
        if file_path:
            print('Load the eval set:' + file_path)
            name = file_path
            truncation_mode = 'keep_end' if name == 'hh' else 'keep_start'
            # Load data from a single file specified by file_path
            for prompt, data in get_dataset(file_path, split, silent=silent, cache_dir=cache_dir).items():
                flat_data.append((prompt, data['responses'], data['pairs'], data['sft_target'], truncation_mode))
        else:
            # Existing logic for loading data from predefined dataset names
            name = names
            truncation_mode = 'keep_end' if name == 'hh' else 'keep_start'

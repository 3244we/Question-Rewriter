...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_name_or_path, cache_dir=get_local_dir(config.local_dirs), use_auth_token=hf_token)
        ...
        self.eval_iterator = get_batch_iterator(**data_iterator_kwargs, file_path=config.eval_dataset_path, split='custom', n_examples=config.n_eval_examples, batch_size=config.eval_batch_size, silent=rank != 0, cache_dir=get_local_dir(config.local_dirs))
    ...
    def evaluate(self):
        if not hasattr(self.config, 'eval_dataset_path'):
            raise ValueError("Evaluation dataset path not configured.")

        eval_dataset = load_dataset(self.config.eval_dataset_path, self.config.eval_batch_size, shuffle=False)
        self.policy.eval() 
        ...
        avg_accuracy = total_accuracy / len(eval_dataset)
    ...
    def train(self):
        ...
        self.best_acc = -float('inf')  # Initialize with the worst possible accuracy
        self.best_loss = float('inf')  # Initialize with the worst possible loss
        self.best_acc_step = None
        self.best_loss_step = None
        self.best_acc_dir = None
        self.best_loss_dir = None
        ...
        self.train_iterator, train_iterator_copy = itertools.tee(self.train_iterator, 2)
        temp_list = list(train_iterator_copy)
        batches_per_epoch = len(temp_list) // self.n_epochs
        ...
        for batch in self.train_iterator:
            ...
            #### EPOCH SAVE ####
            if self.example_counter > 0 and self.batch_counter % batches_per_epoch == 0 and (self.batch_counter // batches_per_epoch) < self.n_epochs:
                    if self.config.debug:
                        rank0_print('skipping save in debug mode')
                    else:
                        output_dir = os.path.join(self.run_dir, f'epoch-{self.batch_counter // batches_per_epoch}')
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                if self.example_counter > 0:
                    ...
                    current_acc = mean_eval_metrics.get('rewards_eval/accuracies', 0)  # Adjust key as needed
                    current_loss = mean_eval_metrics.get('loss/eval', float('inf'))  # Adjust key as needed
                    ...
                    #    self.save(output_dir, mean_eval_metrics)
                        ...
                        # Check and update best accuracy
                        if current_acc > self.best_acc:
                            if self.best_acc_dir and self.rank == 0:  # Check if a best accuracy directory exists
                                shutil.rmtree(self.best_acc_dir)  # Remove the previous best accuracy directory
                            self.best_acc = current_acc
                            best_acc_dir = os.path.join(self.run_dir, f'best-acc-step-{self.example_counter}')
                            self.best_acc_dir = best_acc_dir  # Update the best accuracy directory path
                            self.save(best_acc_dir, mean_eval_metrics)
                            rank0_print(f'Saved new best accuracy checkpoint and deleted the old one: {current_acc} at step {self.example_counter}')

                        # Check and update best loss
                        #if current_loss < self.best_loss:
                        #    if self.best_loss_dir:  # Check if a best loss directory exists
                        #        try:
                        #            shutil.rmtree(self.best_loss_dir)  # Remove the previous best loss directory
                        #        except:
                        #            pass
                        #    self.best_loss = current_loss
                        #    best_loss_dir = os.path.join(self.run_dir, f'best-loss-step-{self.example_counter}')
                        #    self.best_loss_dir = best_loss_dir  # Update the best loss directory path
                        #    self.save(best_loss_dir, mean_eval_metrics)
                        #    rank0_print(f'Saved new best loss checkpoint and deleted the old one: {current_loss} at step {self.example_counter}')
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        if config.lora:
            ...
            policy = policy.to(getattr(torch, config.model.fsdp_policy_mp))
